...
| Modified since | **No.** `git log -- vendor/emdash-skills` shows the import and the move, nothing else |
...
reader which was which. `vendor/` is where this project already keeps external
code it does not own, so that is where this went.
